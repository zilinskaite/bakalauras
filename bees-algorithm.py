...
theta_max = math.pi
num_bees = 130
num_scouts = 15  # Number of scout bees to introduce
...
def plot_ellipse(cx, cy, a, b, theta):
    t = np.linspace(0, 2 * np.pi, 1000)
    x_ellipse = cx + a * np.cos(t) * np.cos(theta) - b * np.sin(t) * np.sin(theta)
    y_ellipse = cy + a * np.cos(t) * np.sin(theta) + b * np.sin(t) * np.cos(theta)
    plt.plot(x_ellipse, y_ellipse, color='red')
...
def artificial_bee_algorithm(file_path, a_max, b_max, theta_max, num_bees, num_scouts, num_onlookers, tolerance, max_iterations):
    ...
    def distance_sum(bee):
        ...
        x1 /= a
        y1 /= b
        distances = np.abs(np.sqrt((np.abs(x1) ** 2) + (np.abs(y1) ** 2)) - 1)
    ...
    return best_bee, best_fitness, best_iteration, dots, height
    ...
    with open(output_file, 'w') as out_f:
        ...
        for file_name in input_files:
            ...
            for run in range(num_runs):
                start_time = time.time()
                best_bee, best_fitness, best_iteration, dots, height = artificial_bee_algorithm(file_path, a_max, b_max,
                                                                                         theta_max, num_bees,
                                                                                         num_scouts, num_onlookers,
            ...
            # Plot the best found ellipse
            cx, cy, a, b, theta = best_bee
            plt.figure()
            plt.scatter(dots[:, 0], dots[:, 1], color='blue', s=3)
            plot_ellipse(cx, cy, a, b, theta)
            plt.xlim(0, max_coord)
            plt.ylim(0, height)
            #plt.title(f"Best Found Ellipse: {file_name}")
            plt.savefig(f'ellipse_plot_{file_name}.png')
            plt.show()
...
input_folder = r'\data\results\test-control'
output_file = r'\data\results\abc-test.txt'
num_runs = 1
